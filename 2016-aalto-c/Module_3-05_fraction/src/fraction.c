...
int compFraction(const Fraction *a, const Fraction *b) {
    int numDifference = a->numerator * b->denominator - b->numerator * a->denominator;
    if (numDifference < 0) return -1;
    else if (numDifference > 0) return 1;
    else return 0;
}
...
void reduceFraction(Fraction* val) {
    int factor = gcd(val->numerator, val->denominator);
